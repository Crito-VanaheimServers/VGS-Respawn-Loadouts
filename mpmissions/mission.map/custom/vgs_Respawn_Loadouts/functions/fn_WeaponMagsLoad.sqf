...
for "_i" from 0 to (count _compatibleMags)-1 do
{
	private _curItem = (_compatibleMags select _i);
	...
	private _itemInfo = [_requiredRespect,_item,_curItem];
...
for "_i" from 0 to (count _PrimaryWpnItms)-1 do
{
	...
	private _item = (_PrimaryWpnItms select _i)select 1;
	private _curItem = (_PrimaryWpnItms select _i)select 2;
	private _itemRespect = getNumber(missionConfigFile >> "CfgExileArsenal" >> _curItem >> "quality");
	private _index = (_display displayCtrl 99108) lbAdd _item;
	(_display displayCtrl 99108) lbSetData [_index,_curItem];

	if((_requiredRespect > _playerRespect) && (vgsLoadoutRep))then
	{
		(_display displayCtrl 99108) lbSetColor [_index,[1,0,0,1]];
		...
		(_display displayCtrl 99108) lbSetTooltip [_index, _toolTip];
	}
	else
	{
		...
		else
		{
			...
			(_display displayCtrl 99108) lbSetColor [_index,_respectColor];
		};
	};
};
